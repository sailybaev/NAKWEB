...
export function Projects() {
	...
	const [buildingTypeKey, setBuildingTypeKey] = useState(
		FILTER_KEYS.ALL_PROJECTS
	)
	...
	const getTranslatedFilter = (key: any) => {
		switch (key) {
			case FILTER_KEYS.ALL_PROJECTS:
				return t('home.projects.filters.allProjects')
			case FILTER_KEYS.UNDER_CONSTRUCTION:
				return t('home.projects.filters.underConstruction')
			case FILTER_KEYS.COMPLETED:
				return t('home.projects.filters.completed')
			case FILTER_KEYS.ALL_TYPES:
				return t('home.projects.filters.all')
			case FILTER_KEYS.APARTMENT:
				return t('home.projects.filters.apartment')
			case FILTER_KEYS.COMMERCIAL:
				return t('home.projects.filters.commercial')
			case FILTER_KEYS.PARKING:
				return t('home.projects.filters.parking')
			case FILTER_KEYS.IN_STOCK:
				return t('home.projects.filters.inStock')
			case FILTER_KEYS.ALL_CLASSES:
				return t('home.projects.filters.allClasses')
			case FILTER_KEYS.ECONOMY:
				return t('home.projects.filters.economy')
			case FILTER_KEYS.COMFORT:
				return t('home.projects.filters.comfort')
			case FILTER_KEYS.COMFORT_PLUS:
				return t('home.projects.filters.comfortPlus')
			case FILTER_KEYS.BUSINESS:
				return t('home.projects.filters.business')
			case FILTER_KEYS.PREMIUM:
				return t('home.projects.filters.premium')
			default:
				return ''
		}
	}
	...
	const handleBuildingTypeChange = (translatedValue: any) => {
		if (translatedValue === t('home.projects.filters.allProjects')) {
			...
			translatedValue === t('home.projects.filters.underConstruction')
		) {
	...
	function filterByPrice(
		...
		const maxPriceNum = maxPrice
			? parseInt(maxPrice.replace(/\s/g, ''))
			: Infinity

		const complexMinPrice = complex.minPriceNumeric

		const apartmentMinPrices = complex.apartments.map(
			(apt: any) => apt.pricePerSqmNumeric
		)

		const lowestPrice = Math.min(
			complexMinPrice,
			...apartmentMinPrices.filter((p: any) => p > 0)
		)
	...
	const filteredProjects = residentialComplexes.filter(complex => {
		const buildingTypeMatch =
			buildingTypeKey === FILTER_KEYS.ALL_PROJECTS ||
			(complex.status === FILTER_KEYS.UNDER_CONSTRUCTION &&
				buildingTypeKey === FILTER_KEYS.UNDER_CONSTRUCTION) ||
			(complex.status === FILTER_KEYS.COMPLETED &&
				buildingTypeKey === FILTER_KEYS.COMPLETED)

		const propertyTypeMatch =
			propertyTypeKey === FILTER_KEYS.ALL_TYPES ||
		...
		const classMatch =
			classTypeKey === FILTER_KEYS.ALL_CLASSES || complex.class === classTypeKey

		const priceMatch = filterByPrice(complex, minPrice, maxPrice)

		const inStockMatch =
			!inStockOnly || complex.apartments.some(apt => apt.available !== false)
	...
	residentialComplexes.forEach(complex => {
		...
		console.log(`Project ${complex.name}:`, {
			...
			inStockMatch,
			passes:
				buildingTypeMatch && propertyTypeMatch && priceMatch && inStockMatch,
		})
	})
